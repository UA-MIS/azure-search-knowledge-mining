		...
		public IActionResult SaveDenyValue(string id, int dislikes)
		{
			...
			async void updateAnnotation()
			{
				...
				else
				{
					...
					//{
					//	await Comments.ExecuteAsync(deleteOperation);
					//}
					//DeleteComment();

					//TableOperation deleteOperation2 = TableOperation.Delete(annotation);
					//async void DeleteAnnotation()
					//{
					//	await Annotations.ExecuteAsync(deleteOperation2);
					//}
					//DeleteAnnotation();
					//remove from annotation table and comment table
					//TableOperation removeOperation = TableOperation.Delete(annotation);
					//await Annotations.ExecuteAsync(removeOperation);
					//fix redisplay on search html******try redirecting to new view for soft deleted stuff to bypass refreshing page with highlighted text
				}
